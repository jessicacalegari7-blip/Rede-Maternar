...
export interface PatientDocument {id:string;patient_id:string;appointment_id:string|null;professional_id:string|null;document_type:'prescription'|'certificate'|'invoice'|'attachment';title:string;content:string|null;document_number:string|null;amount_cents:number|null;status:'draft'|'pending'|'issued'|'cancelled';file_url:string|null;metadata:Record<string,unknown>;issued_at:string|null;created_at:string}
...
export async function createPatientDocument(input:{patientId:string;appointmentId?:string;professionalId?:string;type:PatientDocument['document_type'];title:string;content?:string;number?:string;amount?:number;status?:PatientDocument['status'];fileUrl?:string;metadata?:Record<string,unknown>}){const db=client(),{organizationId,userId}=await getCurrentOrganization();const {data,error}=await db.from('patient_documents').insert({organization_id:organizationId,patient_id:input.patientId,appointment_id:input.appointmentId||null,professional_id:input.professionalId||null,document_type:input.type,title:input.title.trim(),content:input.content?.trim()||null,document_number:input.number?.trim()||null,amount_cents:input.amount==null?null:Math.round(input.amount*100),status:input.status||'issued',file_url:input.fileUrl?.trim()||null,metadata:input.metadata||{},issued_at:(input.status||'issued')==='issued'?new Date().toISOString():null,created_by:userId}).select('*').single();if(error)throw new Error(error.message);return data as PatientDocument}
export async function uploadClinicalAttachment(patientId:string,file:File){if(!['application/pdf','image/jpeg','image/png'].includes(file.type)||file.size>10*1024*1024)throw new Error('Envie PDF, JPG ou PNG de até 10 MB.');const db=client(),{organizationId,userId}=await getCurrentOrganization(),extension=file.name.split('.').pop()?.toLowerCase()||'bin',path=`${organizationId}/${patientId}/${userId}/${crypto.randomUUID()}.${extension}`;const {error}=await db.storage.from('clinical-attachments').upload(path,file,{contentType:file.type,upsert:false});if(error)throw new Error(error.message);return{path,name:file.name,mime:file.type,size:file.size}}
export async function getClinicalAttachmentUrl(path:string){const {data,error}=await client().storage.from('clinical-attachments').createSignedUrl(path,300);if(error)throw new Error(error.message);return data.signedUrl}
