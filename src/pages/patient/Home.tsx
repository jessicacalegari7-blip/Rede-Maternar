...
export function PatientHome(){
  ...
  {professional ? <div className="card" style={{background:'var(--wine)',color:'white'}}><div className="row"><div className="avatar" style={{background:'white'}}>{professional.name.split(' ').slice(0,2).map(p=>p[0]).join('')}</div><div><div style={{opacity:.8}}>Você está sendo acompanhada por</div><h2 style={{margin:'4px 0'}}>{professional.name}</h2><div>{professional.specialty || 'Profissional da MaterPlace'}</div></div></div></div> : <div className="card empty-link-card"><UserRound/><div><h2>Seu vínculo profissional será exibido aqui</h2><p className="muted">Este acesso ainda não possui uma profissional vinculada.</p></div></div>}
