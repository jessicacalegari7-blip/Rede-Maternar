...
export function FinanceSuite({initial='overview'}:{initial?:FinanceView}){
  ...
  const filtered=entries.filter(e=>view==='payable'?['payable','expense'].includes(e.type):view==='receivable'?['receivable','income'].includes(e.type):view==='costs'?e.type==='expense':view==='taxes'?e.type==='tax':view==='payroll'?e.type==='payroll':true)
