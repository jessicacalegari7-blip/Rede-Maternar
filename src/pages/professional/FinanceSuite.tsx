...
export function FinanceSuite({initial='overview'}:{initial?:FinanceView}){
  ...
  const dateFiltered=entries.filter(e=>{const date=(e.due_date||e.paid_at||e.created_at).slice(0,10);return(!dateFrom||date>=dateFrom)&&(!dateTo||date<=dateTo)})
  ...
  const totals=useMemo(()=>{const paid=entries.filter(e=>e.status==='paid');const income=paid.filter(e=>['income','receivable'].includes(e.type)).reduce((s,e)=>s+e.amount_cents,0);const expense=paid.filter(e=>['expense','payable','tax','payroll'].includes(e.type)).reduce((s,e)=>s+e.amount_cents,0);const pending=entries.filter(e=>e.status==='pending'&&['receivable','income'].includes(e.type)).reduce((s,e)=>s+e.amount_cents,0);return{income,expense,pending,result:income-expense}},[entries])
...
function Dre({entries}:{entries:RealFinancialEntry[]}){
  ...
  const now=new Date();const filtered=entries.filter(e=>{const d=new Date(e.paid_at||e.created_at);return e.status==='paid'&&(period==='day'?d.toDateString()===now.toDateString():period==='month'?d.getMonth()===now.getMonth()&&d.getFullYear()===now.getFullYear():d.getFullYear()===now.getFullYear())})
  const revenue=filtered.filter(e=>['income','receivable'].includes(e.type)).reduce((s,e)=>s+e.amount_cents,0)
  const expense=filtered.filter(e=>['expense','payable','tax','payroll'].includes(e.type)).reduce((s,e)=>s+e.amount_cents,0)
  return <><div className="dre-toolbar card"><div><h2>DRE gerencial</h2><p className="muted">Calculada somente com lançamentos pagos.</p></div><div className="tabs compact-tabs"><button className={period==='day'?'active':''} onClick={()=>setPeriod('day')}>Diário</button><button className={period==='month'?'active':''} onClick={()=>setPeriod('month')}>Mensal</button><button className={period==='year'?'active':''} onClick={()=>setPeriod('year')}>Anual</button></div></div><div className="grid grid-3"><Metric icon={CircleDollarSign} label="Receita" value={money.format(revenue/100)}/><Metric icon={ArrowDownLeft} label="Custos e despesas" value={money.format(expense/100)}/><Metric icon={Wallet} label="Resultado líquido" value={money.format((revenue-expense)/100)}/></div></>
}
