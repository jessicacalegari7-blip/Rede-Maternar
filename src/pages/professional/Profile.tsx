...
export function ProfessionalProfilePage() {
  ...
  const lookupLocationPostalCode=async(index:number,value:string)=>{const postal=value.replace(/\D/g,'');if(postal.length!==8){setNotice('CEP inválido. Informe os 8 números.');return}try{setNotice('Consultando CEP…');const response=await fetch(`https://viacep.com.br/ws/${postal}/json/`),address=await response.json();if(!response.ok||address.erro)throw new Error('CEP não encontrado. Verifique os números informados.');setServiceLocations(current=>current.map((item,itemIndex)=>itemIndex===index?{...item,postal_code:postal.replace(/^(\d{5})(\d{3})$/,'$1-$2'),address_line:String(address.logradouro||item.address_line),neighborhood:String(address.bairro||item.neighborhood),city:String(address.localidade||item.city),state_code:String(address.uf||item.state_code).toUpperCase()}:item));setNotice('Endereço localizado. Confira o número e salve as alterações.')}catch(reason){setNotice(reason instanceof Error?reason.message:'Não foi possível consultar o CEP. Preencha o endereço manualmente.')}}
  const save=async()=>{
    setNotice('Salvando e confirmando no banco de dados...');setSaving(true)
    try{await saveProfessionalSpecialties(profile.id,selectedSpecialties,specialtyLimit);const persistedLocations=await saveProfessionalServiceLocations(profile.id,serviceLocations);const first=persistedLocations[0],nextProfile=first?{...profile,address_line:first.address_line,address_number:first.address_number,address_complement:first.address_complement,neighborhood:first.neighborhood,city:first.city,state_code:first.state_code,postal_code:first.postal_code}:profile;const saved=await updateMyProfessionalProfile(profile.id,{...nextProfile,profile_completed:Boolean(nextProfile.full_name&&nextProfile.whatsapp&&selectedSpecialties.length&&persistedLocations.length)});setServiceLocations(persistedLocations);setProfile(saved);setNotice('Alterações salvas com sucesso.')}
  ...
  return <div>
    ...
    <section className="card grid grid-2 profile-editor-grid" onBlurCapture={event=>{const input=event.target as HTMLInputElement,article=input.closest('.service-location-form');if(!article||input.closest('label')?.querySelector('span')?.textContent!=='CEP')return;const index=[...event.currentTarget.querySelectorAll('.service-location-form')].indexOf(article);if(index>=0)void lookupLocationPostalCode(index,input.value)}}>
