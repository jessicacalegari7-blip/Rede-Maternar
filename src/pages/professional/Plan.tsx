...
const prices: Record<ProfessionalPlan,string> = { free:'Plano legado', marketplace:'R$ 9,99/mês', independent:'A partir de R$ 59,90/mês', clinic:'A partir de R$ 99,90/mês', business:'Sob consulta', annual:'Plano legado' }
...
export function ProfessionalPlanPage() {
  ...
  return <div><div className="page-heading"><div><span className="badge">Assinatura</span><h1>{planLabels[plan]}</h1><p className="muted">Escolha o nível de visibilidade e gestão adequado ao seu momento.</p></div></div>
    ...
    <div className="pricing-grid pricing-grid-4 plan-page-grid">{(['free','marketplace','independent','clinic'] as ProfessionalPlan[]).map(item=><article className={`pricing-card ${item==='independent'?'featured':''}`} key={item}><span className="badge">{item===plan?'Plano atual':item==='clinic'?'Para equipes':item==='independent'?'Mais escolhido':'Marketplace'}</span><h3>{planLabels[item]}</h3><div className="price">{prices[item]}</div>{planFeatures[item].map(f=><p className="feature" key={f}><Check size={18}/>{f}</p>)}{item===plan?<div className="alert alert-success">Seu plano está ativo.</div>:<button className="btn btn-primary full" onClick={()=>alert('Contratação simulada. A integração com a Asaas será feita na versão de produção.')}><Sparkles size={17}/> Escolher este plano</button>}</article>)}</div>
  </div>
}
