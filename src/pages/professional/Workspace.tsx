...
import { createPatientAndLead, createPatientRecord, createService, getContactDetail, listLeads, listPatients, listServices, setServiceActive, updateContact, updateLeadStage, updateService, type LeadStage, type RealLead, type RealService } from '../../lib/operations'
...
export function CrmCustomers() {
  ...
  const stage='Primeiro contato'
  const setStage=(_value:string)=>undefined
  ...
  useEffect(()=>{void refresh()},[])
  async function submit(e:React.FormEvent<HTMLFormElement>){e.preventDefault();const data=new FormData(e.currentTarget);try{await createPatientAndLead({name:String(data.get('name')),phone:String(data.get('phone')),email:String(data.get('email')||''),cpf:String(data.get('cpf')||''),source:String(data.get('source')),stage:'new',notes:String(data.get('note')||''),serviceInterest:String(data.get('service')||'')});setSaved(`${data.get('name')} foi gravada no banco e adicionada a Novo contato.`);setShowForm(false);await refresh()}catch(err){setError(err instanceof Error?err.message:'Falha ao salvar cadastro.')}}
  const visiblePatients=patients.filter(patient=>!search||[patient.full_name,patient.phone,patient.email,patient.cpf].some(value=>String(value||'').toLowerCase().includes(search.toLowerCase().replace(/\D/g,''))||String(value||'').toLowerCase().includes(search.toLowerCase())))
