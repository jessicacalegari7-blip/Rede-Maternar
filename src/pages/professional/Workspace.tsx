...
import { createPatientAndLead, createPatientRecord, createService, listPatientRecords, listLeads, listPatients, listServices, setServiceActive, updateLeadStage, updateService, type LeadStage, type RealLead, type RealService } from '../../lib/operations'
...
export function CrmCustomers() {
  ...
  useEffect(()=>{void refresh()},[])
  async function submit(e:React.FormEvent<HTMLFormElement>){e.preventDefault();const data=new FormData(e.currentTarget);const stageMap:Record<string,LeadStage>={'Novo contato':'new','Em conversa':'first_contact_attempt','Proposta enviada':'second_contact_attempt','Agendado':'scheduled','Cliente ativo':'completed'};try{await createPatientAndLead({name:String(data.get('name')),phone:String(data.get('phone')),email:String(data.get('email')||''),cpf:String(data.get('cpf')||''),source:String(data.get('source')),stage:stageMap[stage]||'new',notes:String(data.get('note')||'')});setSaved(`${data.get('name')} foi gravada no banco e adicionada ao CRM.`);setShowForm(false);await refresh()}catch(err){setError(err instanceof Error?err.message:'Falha ao salvar cadastro.')}}
  ...
  async function openRecord(patient:any){setRecordPatient(patient);setRecords(await listPatientRecords(patient.id))}
  return <><div className="page-heading"><div><span className="badge">CRM</span><h1>Clientes</h1><p className="muted">Contatos, histórico comercial, agendamentos e pendências em um só lugar.</p></div><button className="btn btn-primary" onClick={()=>setShowForm(true)}><Plus size={17}/> Cadastrar paciente</button></div>
    ...
    {recordPatient&&<div className="modal-backdrop"><form className="modal-card" onSubmit={async e=>{e.preventDefault();const data=new FormData(e.currentTarget);await createPatientRecord(recordPatient.id,String(data.get('content')));setRecords(await listPatientRecords(recordPatient.id));e.currentTarget.reset()}}><div className="modal-head"><div><h2>Prontuário de {recordPatient.full_name}</h2><p className="muted">Histórico clínico privado da organização.</p></div><button type="button" className="icon-btn" onClick={()=>setRecordPatient(null)}>×</button></div><label className="field"><span>Nova evolução</span><textarea name="content" rows={5} required/></label><button className="btn btn-primary">Salvar no prontuário</button><div className="stack" style={{marginTop:18}}>{records.map(record=><article className="card" key={record.id}><small>{new Date(record.created_at).toLocaleString('pt-BR')}</small><p>{record.content}</p></article>)}</div></form></div>}`r`n
  </>
}
