...
import { defaultEditorialStaff, getEditorialStaff, saveEditorialStaff, type EditorialStaff } from '../../lib/portalSettings'

const emptyArticle: NewsInput = { title: '', seoTitle: '', slug: '', excerpt: '', content: '', category: 'Gestação', coverImageUrl: null, authorName: 'Equipe MaterPlace', status: 'draft', featured: false }
...
export function AdminNewsPortal() {
  ...
  const [staff,setStaff]=useState<EditorialStaff>(defaultEditorialStaff)

  function format(prefix:string,suffix=prefix,placeholder='texto') { if(!editor)return;const start=contentCursor;const value=editor.content;const insertion=`${prefix}${placeholder}${suffix}`;field('content',value.slice(0,start)+insertion+value.slice(start));setContentCursor(start+prefix.length+placeholder.length) }
  ...
  useEffect(() => { void load(); void getEditorialStaff().then(setStaff).catch(()=>undefined) }, [])

  function pasteRichText(event:ClipboardEvent<HTMLTextAreaElement>){
    const html=event.clipboardData.getData('text/html'); if(!html||!editor)return
    event.preventDefault(); const doc=new DOMParser().parseFromString(html,'text/html')
    doc.querySelectorAll('strong,b').forEach(n=>n.replaceWith(`**${n.textContent||''}**`)); doc.querySelectorAll('em,i').forEach(n=>n.replaceWith(`*${n.textContent||''}*`))
    doc.querySelectorAll('a').forEach(n=>n.replaceWith(`[${n.textContent||n.getAttribute('href')}](${n.getAttribute('href')||''})`)); doc.querySelectorAll('h2').forEach(n=>n.replaceWith(`\n\n## ${n.textContent||''}\n\n`)); doc.querySelectorAll('h3').forEach(n=>n.replaceWith(`\n\n### ${n.textContent||''}\n\n`)); doc.querySelectorAll('p,div').forEach(n=>n.append('\n\n'))
  ...
  function formatText(before:string,after=before,placeholder='texto') {
    if(!editor)return
    const start=contentCursor
    field('content',`${editor.content.slice(0,start)}${before}${placeholder}${after}${editor.content.slice(start)}`)
    setContentCursor(start+before.length+placeholder.length+after.length)
  }

  return <div>
    <section className="card editorial-staff-editor"><h2>Expediente do portal</h2><p className="muted">Editável e publicado na página de Expediente.</p><div className="form-grid"><label className="field"><span>Jornalista responsável</span><input value={staff.responsibleJournalist} onChange={e=>setStaff({...staff,responsibleJournalist:e.target.value})}/></label><label className="field"><span>Consultoria técnica em amamentação</span><input value={staff.technicalConsultant} onChange={e=>setStaff({...staff,technicalConsultant:e.target.value})}/></label></div><button className="btn btn-secondary" onClick={()=>void saveEditorialStaff(staff).then(()=>setMessage('Expediente salvo com sucesso.')).catch(e=>setMessage(e.message))}><Save/> Salvar expediente</button></section>
    {editor&&<div className="news-format-toolbar"><strong>Formatar texto:</strong><button type="button" onClick={()=>formatText('**','**','negrito')}><b>Negrito</b></button><button type="button" onClick={()=>formatText('*','*','itálico')}><i>Itálico</i></button><button type="button" onClick={()=>formatText('## ','','Subtítulo')}>Título grande</button><button type="button" onClick={()=>formatText('### ','','Subtítulo')}>Título menor</button><button type="button" onClick={()=>formatText('[grande]','[/grande]','texto maior')}>Fonte maior</button><button type="button" onClick={()=>formatText('[pequeno]','[/pequeno]','texto menor')}>Fonte menor</button></div>}
    ...
    {editor && <div className="modal-backdrop modal-top"><form className="modal-card news-editor" onSubmit={submit}><div className="modal-head"><div><h2>{editor.id ? 'Editar notícia' : 'Nova notícia'}</h2><p className="muted">O conteúdo será exibido dentro do layout da MaterPlace.</p></div><button type="button" className="icon-button" onClick={() => setEditor(null)}><X /></button></div>
      <label className="field"><span>Título SEO (opcional)</span><input value={editor.seoTitle} onChange={event => field('seoTitle', event.target.value)} placeholder="Usado no Google; vazio usa o título do post" /></label>
      <div className="form-grid"><label className="field span-2"><span>Título</span><input required value={editor.title} onChange={event => { field('title', event.target.value); if (!editor.id) field('slug', slugify(event.target.value)) }} /></label><label className="field"><span>URL amigável</span><input required value={editor.slug} onChange={event => field('slug', slugify(event.target.value))} /></label><label className="field"><span>Categoria</span><input required value={editor.category} onChange={event => field('category', event.target.value)} /></label><label className="field span-2"><span>Resumo</span><textarea required rows={3} value={editor.excerpt} onChange={event => field('excerpt', event.target.value)} /></label><label className="field span-2"><span>Texto da matéria</span><div className="rich-toolbar" role="toolbar" aria-label="Formatação"><button type="button" onClick={()=>format('## ','','Subtítulo')}>Título</button><button type="button" onClick={()=>format('### ','','Seção')}>Subtítulo</button><button type="button" onClick={()=>format('**','**','negrito')}><b>B</b></button><button type="button" onClick={()=>format('*','*','itálico')}><i>I</i></button><button type="button" onClick={()=>format('- ','','item da lista')}>Lista</button><button type="button" onClick={()=>format('> ','','citação')}>Citação</button><button type="button" onClick={()=>format('[','](https://)','texto do link')}>Link</button></div><textarea required rows={13} value={editor.content} onPaste={pasteRichText} onSelect={event=>setContentCursor(event.currentTarget.selectionStart)} onChange={event => field('content', event.target.value)} placeholder="Escreva o conteúdo e use a barra para formatar." /><span className="editor-upload-row"><label className="btn btn-secondary"><ImagePlus/> Inserir imagem aqui<input type="file" accept="image/*" hidden onChange={event=>void upload(event.target.files?.[0],'body')}/></label><small>A formatação é salva como Markdown seguro e permanece disponível ao reabrir.</small></span></label><label className="field"><span>Autor</span><input required value={editor.authorName} onChange={event => field('authorName', event.target.value)} /></label><label className="field"><span>Imagem de capa</span><label className="news-cover-upload"><Upload/>{uploading?'Enviando...':'Escolher imagem'}<input type="file" accept="image/jpeg,image/png,image/webp" hidden disabled={uploading} onChange={event=>void upload(event.target.files?.[0],'cover')}/></label><small>Ideal: 1600 × 900 px, JPG/WebP, até 500 KB.</small>{editor.coverImageUrl&&<img src={editor.coverImageUrl} alt="Prévia da capa"/>}</label><label className="field"><span>Status</span><select value={editor.status} onChange={event => field('status', event.target.value as NewsStatus)}><option value="draft">Rascunho</option><option value="published">Publicada</option><option value="archived">Arquivada</option></select></label><label className="news-featured-check"><input type="checkbox" checked={editor.featured} onChange={event => field('featured', event.target.checked)} /> Destacar na home</label></div>
