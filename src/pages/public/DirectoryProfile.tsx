...
export function DirectoryProfile(){
  ...
  const schema=useMemo(()=>{
    ...
    if(row.professional_registration)result.identifier={'@type':'PropertyValue',propertyID:row.professional_registration.split(/\s|-/)[0],value:row.professional_registration}
    if(row.plan_type==='premium'&&row.is_claimed&&row.whatsapp)result.telephone=row.whatsapp
    if(row.review_count&&row.rating)result.aggregateRating={'@type':'AggregateRating',ratingValue:row.rating,reviewCount:row.review_count}
    return result
  },[row])
  ...
  return <div className="clinic-profile-page">
    ...
    <main className="clinic-profile-shell"><section className="card directory-profile-card">
      ...
      {row.professional_registration&&<p><strong>Registro profissional:</strong> {row.professional_registration}</p>}
