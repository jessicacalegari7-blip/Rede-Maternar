...
type SeoProps = { title:string; description:string; path?:string; image?:string|null; type?:'website'|'article'; schema?:Record<string,unknown>|Array<Record<string,unknown>>; appendBrand?:boolean; index?:boolean }
...
export function Seo({title,description,path=window.location.pathname,image,type='website',schema,appendBrand=true,index=true}:SeoProps){
  useEffect(()=>{
    ...
    meta('meta[name="description"]',{name:'description',content:description}); meta('meta[name="robots"]',{name:'robots',content:index?'index, follow, max-image-preview:large, max-snippet:-1, max-video-preview:-1':'noindex, follow'})
  ...
  },[title,description,path,image,type,schema,appendBrand,index]); return null
}
