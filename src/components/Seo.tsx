...
type SeoProps = { title:string; description:string; path?:string; image?:string|null; type?:'website'|'article'; schema?:Record<string,unknown>|Array<Record<string,unknown>> }
...
export function Seo({title,description,path=window.location.pathname,image,type='website',schema}:SeoProps){
  useEffect(()=>{
    const canonical=new URL(path,ORIGIN).toString(); const shareImage=image?new URL(image,ORIGIN).toString():DEFAULT_IMAGE
    document.title=`${title} | MaterPlace`
  ...
  },[title,description,path,image,type,schema]); return null
}
