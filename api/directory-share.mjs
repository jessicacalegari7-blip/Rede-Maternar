...
function page({title,description,canonical,schema,content}){
  return `<!doctype html><html lang="pt-BR"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escape(title)}</title><meta name="description" content="${escape(description)}"><meta name="robots" content="index, follow, max-image-preview:large, max-snippet:-1, max-video-preview:-1"><link rel="canonical" href="${escape(canonical)}"><meta property="og:title" content="${escape(title)}"><meta property="og:description" content="${escape(description)}"><meta property="og:image" content="${LOGO}"><meta property="og:url" content="${escape(canonical)}"><meta property="og:type" content="website"><meta name="twitter:card" content="summary_large_image"><meta name="twitter:title" content="${escape(title)}"><meta name="twitter:description" content="${escape(description)}"><meta name="twitter:image" content="${LOGO}"><script type="application/ld+json">${json(schema)}</script></head><body><header><a href="${ORIGIN}"><img src="${LOGO}" width="320" height="160" alt="MaterPlace"></a></header><main>${content}</main><footer><a href="${ORIGIN}/expediente">Expediente</a> · <a href="${ORIGIN}/privacidade">Privacidade</a> · <a href="${ORIGIN}/contato">Contato</a></footer></body></html>`
}
...
export default async function handler(request,response){
  try{
    ...
    if(profileId){
      ...
      if(!data)return response.status(404).send(page({title:'Perfil não encontrado | MaterPlace',description:'O perfil solicitado não está disponível.',canonical:`${ORIGIN}${request.url}`,schema:{'@context':'https://schema.org','@type':'WebPage'},content:'<h1>Perfil não encontrado</h1>'}))
    ...
    return response.status(200).send(page({title,description,canonical,schema,content:`<h1>${escape(specialtyName)} em ${escape(cityName)}, ${escape(state)}</h1>${cards||'<p>Novos profissionais estão em validação.</p>'}`}))
