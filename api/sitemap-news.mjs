...
export default async function handler(_request,response){
  ...
  const urls=(data||[]).map(item=>`<url><loc>https://www.materplace.com.br/noticias/${escapeXml(item.slug)}</loc><news:news><news:publication><news:name>MaterPlace</news:name><news:language>pt-BR</news:language></news:publication><news:publication_date>${escapeXml(item.published_at)}</news:publication_date><news:title>${escapeXml(item.title)}</news:title></news:news></url>`).join('')
